...
@section('content')
    <div class="mx-auto max-w-4xl space-y-6">
        <div class="mm-surface p-6 sm:p-8">
            ...
            <h1 class="mt-2 mm-section-title">Edit product</h1>
            <p class="mm-section-copy">Update the product details for {{ $product->name }}.</p>
        </div>

        <div class="mm-surface p-6 sm:p-8">
            <form method="post" action="{{ route('admin.products.update', $product) }}" class="space-y-5">@csrf @method('put')
                <div>
                    <label class="mm-label" for="product-name">Product name</label>
                    <input id="product-name" name="name" value="{{ old('name', $product->name) }}" class="mm-input mt-2">
                ...
                <div>
                    ...
                    <textarea id="product-description" name="description" rows="5" class="mm-input mt-2">{{ old('description', $product->description) }}</textarea>
                </div>

                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label class="mm-label" for="product-price">Price</label>
                        <input id="product-price" name="price" value="{{ old('price', $product->price) }}" class="mm-input mt-2">
                    ...
                    <div>
                        ...
                        <input id="product-quantity" name="quantity" value="{{ old('quantity', $product->quantity) }}" class="mm-input mt-2">
                ...
                <div>
                    ...
                    <input id="image_url" name="image_url" value="{{ old('image_url', $product->image_url) }}" class="mm-input mt-2">
                    @if($errors->has('image_url'))<div class="mt-2 text-sm text-rose-600">{{ $errors->first('image_url') }}</div>@endif
                    <img id="preview" src="{{ $product->image_url }}" class="mt-4 h-44 w-full rounded-3xl object-cover {{ $product->image_url ? '' : 'hidden' }}">
                </div>

                <button class="mm-btn-primary">Update product</button>
            </form>
        </div>
    </div>
